...
export const backendEnvSchema = z
  .object({
    ...
    ATTACHMENT_ANALYSIS_ENABLED: z.preprocess(parseBoolean, z.boolean()).default(true),
    ATTACHMENT_MAX_BYTES: z.coerce.number().int().positive().max(5 * 1024 * 1024).default(1_048_576),
    ATTACHMENT_TEXT_MAX_CHARS: z.coerce.number().int().positive().max(60_000).default(8_000),
    ATTACHMENT_ALLOWED_MIME_TYPES: z.string().default("text/plain,text/markdown,application/json"),
    ATTACHMENT_ALLOW_PDF_TEXT: z.preprocess(parseBoolean, z.boolean()).default(false),
    STT_PROVIDER: z.enum(["mock", "vosk", "whisper_cpp", "openai_whisper"]).default("mock"),
    STT_FALLBACK_PROVIDER: z.enum(["mock"]).default("mock"),
    STT_MAX_AUDIO_BYTES: z.coerce.number().int().positive().max(15 * 1024 * 1024).default(5_242_880),
    STT_MAX_DURATION_SECONDS: z.coerce.number().int().positive().max(300).default(30),
    STT_PROVIDER_TIMEOUT_MS: z.coerce.number().int().positive().default(20_000),
